...
export type RouteConfig = { profiles: string[]; timeoutMs: number; maxAttempts?: number };

const retryableFailures = new Set<ProviderFailureCode>([
  'rate_limit',
  'timeout',
  'invalid_response',
  'upstream',
...
export class EnrichmentRegistry {
  ...
  constructor(
    ...
    for (const [method, route] of Object.entries(routes)) {
      if (
        ...
        (route.maxAttempts !== undefined &&
          (!Number.isInteger(route.maxAttempts) || route.maxAttempts < 1 || route.maxAttempts > 3))
      )
  ...
  async enrich(
    ...
    for (const id of route.profiles) {
      ...
      const maxAttempts = route.maxAttempts ?? 2;
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          ...
          output = await Promise.race([
            ...
            new Promise<never>((_resolve, reject) => {
              timer = setTimeout(
                ...
                Math.min(profile.timeoutMs, Math.max(1, Math.floor(remaining / attemptsLeft))),
              );
            }),
          ]);
          status = 'succeeded';
        ...
        if (!lastFailure || !retryableFailures.has(lastFailure)) break;
      }
    }
    return attempted
